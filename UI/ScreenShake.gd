...
var amplitude = 0 # how far from the center the camera will move
var priority = 0 # priority is to take larger exposions rather than smaller ones
...
func start(duration = 0.2, frequency = 15, amplitude = 16, priority = 0):
	if (priority >= self.priority):
		...
		$Duration.wait_time = duration # how long shake will last in seconds
